...
- `Dashboard` — KPIs, revenue/profit (date-range picker), production charts (recharts), P&L, dues. Product sales counted in revenue/cash only when `paid=true`.
...
- `Bills` — monthly bill generation, Razorpay link, PDF, WhatsApp send, mark-paid. Core logic in `lib/bills.js`.
...
- `utils.js` — `formatQty`, `formatCurrency`, `formatDate`, `getMonthBounds`, `currentYearMonth`, `todayISO`, `isOverdue`, `getBillStatus`, `statusBadgeClass`, `cleanPhone`, `whatsappLink`, `last6Months`, `last30Days`, PDF-specific formatters.
